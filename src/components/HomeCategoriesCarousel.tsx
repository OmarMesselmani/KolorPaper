...
export default function HomeCategoriesCarousel({ categories }: HomeCategoriesCarouselProps) {
  ...
  return (
    <div className="w-full max-w-[1240px] mx-auto px-6 mt-12 mb-16 relative">
      ...
      {/* 20 Category Cards in 5 columns per row */}
      <div className="grid grid-cols-5 gap-2.5 sm:gap-4 md:gap-6">
        {displayCategories.map((category, idx) => (
          <CategoryCard key={`${category.id}-${idx}`} category={category} index={idx} />
        ))}
      </div>
