...
// GET /api/pages
export const getColoringPages = async (req, res) => {
    try {
...
// GET /api/pages/:slug
export const getColoringPageBySlug = async (req, res) => {
    try {
...
export const createColoringPage = async (req, res) => {
    try {
        const { title, slug, imageUrl, thumbnailUrl, pdfUrl, categorySlug, subCategorySlug, description, difficulty, ageGroup, tags, published } = req.body;
        ...
        const cleanTags = Array.isArray(tags) ? tags.map((t) => stripHtml(t).substring(0, 50)) : [];
...
export const updateColoringPage = async (req, res) => {
    try {
        ...
        const cleanTags = tags !== undefined ? (Array.isArray(tags) ? tags.map((t) => stripHtml(t).substring(0, 50)) : []) : existing.tags;
...
export const deleteColoringPage = async (req, res) => {
    try {
        const id = req.params.id;
